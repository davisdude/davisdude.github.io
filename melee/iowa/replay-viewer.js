(function() {
    ...
    function createReplayItem(replay) {
        ...
        div.innerHTML = `
            ...
            <div class="replay-video" data-loaded="false" data-youtubeid="${replay.youtubeId}">
    ...
    function unloadVideo(videoContainer) {
        ...
        const wrapper = videoContainer.querySelector('.video-wrapper');
        wrapper.innerHTML = '';
        videoContainer.dataset.loaded = 'false';
    }

    // Display replays with pagination
    function displayReplays(replays, page = 1) {
        // Clear existing items
        resultsContainer.querySelectorAll('.replay-item').forEach(el => el.remove());

        // Update current search results
        currentSearchResults = replays;

        // Calculate pagination
        totalPages = Math.max(1, Math.ceil(replays.length / config.pageSize));
        currentPage = Math.min(Math.max(1, page), totalPages);

        // Get items for current page
        const start = (currentPage - 1) * config.pageSize;
        const end = start + config.pageSize;
        const pageItems = replays.slice(start, end);
        ...
        if (pageItems.length === 0) {
            ...
            pageItems.forEach(replay => {
                resultsContainer.appendChild(createReplayItem(replay));
            });
        }
